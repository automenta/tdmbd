...
SEQ_LEN = 32                        # Sequence length during training/eval
...
class SelectiveScan(nn.Module):
    ...
    def __init__(self, embed_dim: int, state_dim: int = 16, d_conv: int = 4, dt_rank: Union[str, int] = 'auto', bias=False):
        ...
        with torch.no_grad():
            ...
            target_dt = 0.001
            inv_softplus_target = math.log(math.expm1(target_dt)) if target_dt > 0 else -10.0
            self.dt_proj.bias.data.fill_(inv_softplus_target)

    def _compute_A_tilde(self, dt: Tensor, A_log: Tensor) -> Tensor:
        A = -torch.exp(A_log.float())
        A_tilde = torch.exp(dt.float().unsqueeze(-1) * A.unsqueeze(0).unsqueeze(1))
        return A_tilde
    ...
    def _compute_B_tilde(self, dt: Tensor, A_log: Tensor, A_tilde: Optional[Tensor] = None) -> Tensor:
        ...
        dt_float = dt.float()
        if A_tilde is None:
            A_tilde = self._compute_A_tilde(dt_float, A_log)
        A_unsqueezed = A.unsqueeze(0).unsqueeze(1)
        is_small_A = torch.abs(A_unsqueezed) < 1e-5
        B_tilde = torch.where(
            is_small_A, dt_float.unsqueeze(-1),
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # 1. Input Projections (Keep x_res path implicit for now)
        # x_res = self.in_proj_x(x) # Not explicitly used until final block output in Mamba
        z = self.in_proj_z(x)
        ...
        # 3. Conv + SiLU Activation -> 'u'
        x_conv = self.conv1d(x.transpose(1, 2))[:, :, :seq_len]
        u = F.silu(x_conv.transpose(1, 2))

        # 4. Compute SSM Parameters
        A_tilde = self._compute_A_tilde(dt, self.A_log)
        B_tilde = self._compute_B_tilde(dt, self.A_log, A_tilde)

        # 5. Sequential Scan
        h = torch.zeros(batch, self.embed_dim, self.state_dim, device=x.device, dtype=torch.float32)
        ys: List[Tensor] = []
        for t in range(seq_len):
            A_t = A_tilde[:, t]
            B_t = B_tilde[:, t]
            # B_proj_t = B_proj[:, t] # Not used in this recurrence interpretation
            C_proj_t = C_proj[:, t]
            u_t = u[:, t]

            # Recurrence: h_t = A_t * h_{t-1} + B_t * u_t
            input_term = B_t * u_t.unsqueeze(-1)
            h = A_t * h + input_term

            # Output: y_t = C_proj_t * h_t
    ...
    def step(self, x_step: Tensor, h_prev: Tensor, conv_state: Tensor) -> Tuple[Tensor, Tensor, Tensor]:
        ...
        # 1. Convolution Step -> u_step, new_conv_state
        conv_input = torch.cat([conv_state, x_step.unsqueeze(2)], dim=2)
        conv_weight = self.conv1d.weight.to(dtype=conv_input.dtype)
        conv_bias = self.conv1d.bias.to(dtype=conv_input.dtype) if self.conv1d.bias is not None else None
        conv_out = F.conv1d(conv_input, weight=conv_weight, bias=conv_bias, groups=self.embed_dim, padding=0).squeeze(-1)
        new_conv_state = conv_input[:, :, 1:]
        u_step = F.silu(conv_out)

        # 2. Input Projections for Step -> z_step, dt_unproj, B_proj, C_proj
        ...
        # 4. Compute Discretized A_tilde_step, B_tilde_step
        dt_for_compute = dt_step.unsqueeze(1)
        A_log_float = self.A_log.to(dtype=torch.float32)
        A_tilde_step = self._compute_A_tilde(dt_for_compute, A_log_float).squeeze(1)
        B_tilde_step = self._compute_B_tilde(dt_for_compute, A_log_float, A_tilde_step.unsqueeze(1)).squeeze(1)

        # 5. Update SSM State: h_new = A_tilde_step * h_prev + B_tilde_step * u_step
        input_term = B_tilde_step * u_step.unsqueeze(-1)
        h_new = A_tilde_step * h_prev.to(dtype=torch.float32) + input_term

        # 6. Compute Output: y = C_proj * h_new
        y = torch.einsum('bn, bdn -> bd', C_proj, h_new)

        # 7. Add Skip Connection: y_ssm_out = y + u_step * D
        y_ssm_out = y + u_step * self.D.unsqueeze(0)

        # 8. Apply Gating: y_gated = y_ssm_out * silu(z_step)
        y_gated = y_ssm_out * F.silu(z_step)

        # 9. Final Output Projection: y_step = out_proj(y_gated)
...
def generate_sample_text(model: nn.Module, tokenizer: AutoTokenizer, prompt_text: str, device: torch.device, model_type: str, max_new: int = 48, gen_kwargs: Optional[Dict] = None) -> str:
    ...
    try:
        ...
        if model_type == "hydra":
            default_sampling_steps = min(getattr(model, 'num_timesteps', 100) // 4, 25)
...
def objective(trial: optuna.Trial, train_data: List[Tensor], eval_data: List[Tensor], fixed_seq_len: int, device: torch.device) -> Tuple[float, float]:
    ...
    try:
        ...
        embed_dim_options = [d for d in range(128, 768 + 1, 64) if d % 8 == 0]
        ...
        trial.set_user_attr("param_count_M", param_count_M)
        ...
        else: print(f"   📝 Skipping text generation (Loss: {final_loss:.4f}, Acc: {final_accuracy:.4f}).")
        ...
        if trial_status == "COMPLETE": print(f"--- ✅ Trial {trial.number} Complete: Acc={final_accuracy:.4f}, Params={param_count_M:.3f}M ---")
